...
class Product:
    ...
    def reduce_stock(self,quantity):
        if self.__stock >= quantity:
            ...
            print(f"{self.name} 재고 {quantity}만큼 차감! 남은 수량: {self.__stock}개")
        else:
            print(f"{self.name} 재고 부족!")
    
    def add_stock(self,quantity):
        self.__stock += quantity
        print(f"{self.name} 재고 {quantity}개 추가! 남은 수량 : {self.__stock}개")
...
class Cart:
    ...
    def remove_product(self, product_name):
        for index, (product, qty) in enumerate(self.item):
